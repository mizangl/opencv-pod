...
@interface VTrueDepthEstimator : NSObject <VBaseDepthEstimator>
...
/**
    @abstract Set the depth data.
        Must be called from the main thread.
    @param depthData    AV depth data as received from the iOS SDK.
*/
- (void)setDepthData:(nonnull AVDepthData*)depthData API_AVAILABLE(ios(11.0));

/**
    @abstract Set the target region for deepest point reporting.
        Must be called from the main thread.

    @param measurementJSON    Measurement JSON string containing the analysis on the main/target image.
*/
- (void)setTargetMeasurement:(nonnull NSString*)measurementJSON;
